      ...
      <div class="container mx-auto flex justify-between items-center px-4">
        <div class="flex-shrink-0">
          <img
            ...
            class="h-24 md:h-28 lg:h-32 rounded-full transition-transform duration-300 hover:scale-105"
          />
        </div>

<nav class="hidden md:flex space-x-8 items-center mr-27">
  <router-link to="/" class="text-white hover:text-gray-400 uppercase font-medium">Home</router-link>
  <router-link to="/about" class="text-white hover:text-gray-400 uppercase font-medium">About</router-link>
  
<div class="relative group">
  <router-link to="/gallery" class="text-white hover:text-gray-400 uppercase font-medium">
    Artists
  </router-link>
<div
  class="absolute hidden group-hover:block 
         text-white pt-6 pb-2 w-48 rounded-lg shadow-lg z-50 -mt-5 left-1/2 transform -translate-x-1/2 text-center"
>
  <router-link to="/gallery/artists/francis-field" class="block py-2 hover:text-gray-400 transition-all duration-300">Cara Nicolas</router-link>
  <router-link to="/gallery/artists/mike-goce" class="block py-2 hover:text-gray-400 transition-all duration-300">Leo Botulan</router-link>
  <router-link to="/gallery/artists/pablo-tobias" class="block py-2 hover:text-gray-400 transition-all duration-300">Ivan Nicolas</router-link>
  <router-link to="/gallery/artists/phok-isip" class="block py-2 hover:text-gray-400 transition-all duration-300">JR Barro</router-link>
</div>

</div>

  <router-link to="/tattoo" class="text-white hover:text-gray-400 uppercase font-medium">Tattoo</router-link>
  <router-link to="/artist" class="text-white hover:text-gray-400 uppercase font-medium">Piercing</router-link>
  <router-link to="/shop" class="text-white hover:text-gray-400 uppercase font-medium">Shop</router-link>
  <router-link to="/contactt" class="text-white hover:text-gray-400 uppercase font-medium">Contact</router-link>

  <router-link
    to="/contactt"
    class="py-2 px-4 border border-white rounded-full text-white hover:bg-white hover:text-black transition-all font-semibold uppercase"
...
const galleryMobileOpen = ref(false); // Bagong state para sa mobile gallery

function toggleMenu() {
  menuOpen.value = !menuOpen.value;
  // Isara ang mobile gallery dropdown pag binuksan ang main menu
